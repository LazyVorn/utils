...
<script>
import TaskTreeElement from "./TaskTreeElement";
export default {
  name: "TaskTable",
  props: {
  ...
  methods: {
    ...
    getDateArr() {
      let _arr = [],
        _mark = this.changeDate(this.startTime),
        end = this.changeDate(this.endTime);
      while (_mark <= end) {
        _arr.push(new Date(this.changeDate(_mark)));
        _mark += 3600 * 24 * 1000;
      }
      this.dateArr = _arr;
    },
      ...
      let _arr = this.headData.map((ele, index) => {
        if (ele.width && ele.width != "") {
          ...
          if (index == this.headData.length - 1) {
            return _tableWidth - _totalWidth + this.scrollWidth;
          } else {
