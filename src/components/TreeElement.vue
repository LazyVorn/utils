    ...
    <div class="lv_tree_element_wrap">
        <div class="lv_tree_element_table" v-for="info in treeData" :key="info[idName]">
            <div class="lv_tree_table_title" @click.stop="trClick(info)" :class="[info.isClicked ? 'active' : '',`layer${treeLayer}`]">
                <template v-for="(ele,index) in order">
                    ...
                    :title="info[order[index]]" @click.stop="trClick(info,order[index])">
                        <i class="lv_icon" v-if="(info.childNode) || info[pIdName] ==''" 
                        :class="info.isShow ? 'lv_icon_show' : 'lv_icon_hide'" @click.stop="arrowClick(info)"></i>
                        {{info[order[index]]}}
                    </span>
            ...
            <div class="lv_tree_table_box" v-show="info.isShow">
                <TreeElement v-if="info.childNode" :treeData="info.childNode" :pIdName="pIdName" :order="order"
                :choosedType="choosedType" :width="width" :treeLayer="treeLayer + 1" :headData="headData"
                :left="parseInt(left)+25" @getTrClick="getTrClick" @getChooseBox="getChooseBox"></TreeElement>
            </div>
        </div>
    </div>
</template>
...
<script>export default {
    ...
    methods: {
        ...
        //点击勾选框
        checkboxFunc(e) {
            let _status = !e.isChoosed;
            e.isChoosed = _status;
            e.halfChoosed = _status;
            if (this.choosedType == "normal") {
                !e.childNode ?
                    "" :
                    e.childNode.forEach(ele => {
                        ele.isChoosed = _status;
                        ele.halfChoosed = _status;
        ...
        getChooseBox(pid) {
            ...
            _obj.halfChoosed = (_num > 0 || _half > 0) ? true : false;
            this.$forceUpdate();
            if (!_obj[_pId] || _obj[_pId] == "") {
                this.$emit("getChooseBox", null);
            } else {
                this.$emit("getChooseBox", _obj[_pId]);
            }
        }
    },
    mounted() {
